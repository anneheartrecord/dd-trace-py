...
memalloc_alloc(int use_calloc, void* ctx, size_t nelem, size_t elsize)
{
    ...
    if (ptr) {
        ...
        if (memalloc_heap_sample_check_no_cpython(nelem * elsize, memalloc_ctx->domain, &allocated_memory_val)) {
...
memalloc_realloc(void* ctx, void* ptr, size_t new_size)
{
    ...
    if (ptr2) {
        memalloc_heap_untrack_no_cpython(ptr);
        uint64_t allocated_memory_val = 0;
        if (memalloc_heap_sample_check_no_cpython(new_size, memalloc_ctx->domain, &allocated_memory_val)) {
            memalloc_heap_track_sample_invokes_cpython(
